...
{
    [DefaultPriority(0)]
    public class RouteTypeTests : IClassFixture<RouteTypesFixture>
    {
        ...
        private readonly RouteTypesFixture _fixture;

        public RouteTypeTests(RouteTypesFixture fixture, IServer server, ITestOutputHelper output)
        {
            _fixture = fixture;
            _output = output;
            ...
            var r4mApi4Service = new Route4MeApi4Service(_httpClient);
            r4mController = new Route4MeApi4Controller(r4mApi4Service, _httpClient);
        }

        [FactSkipable]
        public async Task MultipleDepotMultipleDriverTest()
        {
            ...
            if ((response.Item1?.OptimizationProblemId?.Length ?? 0) == 32)
                _fixture.removeOptimizationsId.Add(response.Item1.OptimizationProblemId);

            //if ((response.Item1?.OptimizationProblemId?.Length ?? 0) == 32)
            //    _fixture.tdr.RemoveOptimization(new string[] { response.Item1.OptimizationProblemId });

            Assert.True(response.Item1.Routes.Length > 1);
        }

        [FactSkipable]
        public async Task MultipleSeparateDepostMultipleDriverTest()
        {
            #region Depots
        ...
        [FactSkipable]
        public async Task MultipleDepotMultipleDriverTimeWindowTest()
        {
            // Prepare the addresses
            Address[] addresses = new Address[]
              {
        ...
        [FactSkipable]
        public async Task SingleDepotMultipleDriverNoTimeWindowTest()
        {
            // Prepare the addresses
            Address[] addresses = new Address[]
              {
        ...
        [FactSkipable, Priority(11)]
        public async Task MoveDestinationToRouteTest()
        {
            var optimizationParameters = new OptimizationParameters()
            {
                OptimizationProblemID = _fixture.dataObjectMDMD24.OptimizationProblemId
            };

            var optimizationResult = await r4mController.GetOptimization(optimizationParameters);

            Assert.NotNull(optimizationResult);

            Assert.True(
                optimizationResult.Item1.Routes.Length >= 2,
                "There is no 2 routes for moving a destination to other route.");

            var route1 = optimizationResult.Item1.Routes[0];

            Assert.True(route1.Addresses.Length >= 2,
            ...
            Assert.True(routeDestinationIdToMove > 0, "Wrong destination_id to move: " + routeDestinationIdToMove);

            var route2 = optimizationResult.Item1.Routes[1];

            Assert.True(route1.Addresses.Length >= 2, "There is less than 2 addresses in the generated route...");
            ...
            Assert.True(afterDestinationIdToMoveAfter > 0, 
                "Wrong destination_id to move after: " + afterDestinationIdToMoveAfter);

            var result = await r4mController.MoveDestinationToRoute(route2.RouteId, 
                routeDestinationIdToMove, 
                afterDestinationIdToMoveAfter
                );

            Assert.NotNull(result);
            Assert.IsType<MoveDestinationToRouteResponse>(result.Item1);
            Assert.True(result.Item1.Success, 
                $"MoveDestinationToRouteTest failed. Error: {result.Item1.Error}");
        }

        [FactSkipable, Priority(10)]
        public async Task MergeRoutesTest()
        {
            var route1 = _fixture.dataObjectMDMD24.Routes[0];

            Assert.True(route1.Addresses.Length >= 2,
                "There is less than 2 addresses in the generated route.");

            var route2 = _fixture.dataObjectMDMD24.Routes[1];
            ...
            var response = await r4mController
                .MergeRoutes(mergeRoutesParameters);

            Assert.NotNull(response);
            Assert.True(response.Item1.Status, "MergeRoutesTest failed.");
            Assert.True((response.Item1?.OptimizationProblemId?.Length ?? 0)==32, "MergeRoutesTest failed.");

            _fixture.tdr.RemoveOptimization(new string[] { response.Item1.OptimizationProblemId });
        }

        [FactSkipable]
        public async Task TruckingSingleDriverMultipleTimeWindowsTest()
        {
            // Prepare the addresses
            Address[] addresses = new Address[]
              {
        ...
        [FactSkipable]
        public async Task SingleDriverMultipleTimeWindowsTest()
        {
            // Prepare the addresses
            Address[] addresses = new Address[]
              {
            ...
            Assert.True(response.Item1.Routes.Length > 0);
        }

        [FactSkipable]
        public async Task OptimizationByOrderTerritoriesTest()
        {
            // Set parameters
            var parameters = new RouteParameters()
            {
            ...
            var response = await r4mController
                .RunOptimizationByOrderTerritories(optimizationParameters);

            Assert.NotNull(response);
            Assert.IsType<DataObject[]>(response.Item1);

            var terrOptimizationIDs = new List<string>();

            foreach (var terOpt in response.Item1)
            {
                if ((terOpt?.OptimizationProblemId?.Length ?? 0) == 32) terrOptimizationIDs.Add(terOpt.OptimizationProblemId);
            }

            foreach (var optId in terrOptimizationIDs) _fixture.removeOptimizationsId.Add(optId);

            //_fixture.tdr.RemoveOptimization(terrOptimizationIDs.ToArray());
        }

        [FactSkipable]
        public async Task BundledAddressesTest()
        {
            if (!_fixture.hasCommercialCapabalities) return;
        ...
        public async Task GetScheduleCalendarTest()
        {
            ...
            var result = await r4mController.GetScheduleCalendar(calendarQuery);

            Assert.True(result!=null, "The test GetScheduleCalendarTest failed");
            Assert.IsType <ScheduleCalendarResponse>(result.Item1);

            Assert.True((result.Item1?.AddressBook ?? null)!=null, "The test GetScheduleCalendarTest failed");
            Assert.True((result.Item1?.Orders ?? null) != null, "The test GetScheduleCalendarTest failed");
            Assert.True((result.Item1?.RoutesCount ?? null) != null, "The test GetScheduleCalendarTest failed");
        }

        [FactSkipable]
        public async Task SingleDriverRoundTripTest()
        {
            // Prepare the addresses
            ...
            {
                AlgorithmType = AlgorithmType.TSP,
                RouteName = "Single Driver Round Trip "+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
            ...
            //if ((response.Item1?.OptimizationProblemId?.Length ?? 0) == 32) 
            //    tdr.RemoveOptimization(new string[] { response.Item1.OptimizationProblemId });
            ...
            //if ((response.Item1?.OptimizationProblemId?.Length ?? 0) == 32)
            //    _fixture.dataObject = response.Item1;
        }

        [FactSkipable, Priority(-10)]
        public async Task MultipleDepotMultipleDriverWith24StopsTimeWindowTest()
        {
            // Prepare the addresses
            Address[] addresses = new Address[]
              {
            ...
            _fixture.dataObjectMDMD24 = response.Item1;
        ...
        [FactSkipable]
        public async Task RunOptimizationSingleDriverRoute10StopsTest()
        {
            // Prepare the addresses
            Address[] addresses = new Address[]
            {
        ...
        [FactSkipable]
        public async Task Route300StopsTest()
        {
            Address[] addresses = new Address[]
                {
                    #region Addresses
            ...
            //var response = r4mController
            //    .RunOptimization(optimizationParameters).Result;
