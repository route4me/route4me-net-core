...
namespace Route4MeSDK
{
	...
	public sealed class Route4MeManager
	{
		...
		public Address UpdateOptimizationDestination(Address addressParameters, out string errorString)
		{
			...
			var dataObject = GetJsonObjectFromAPI<DataObject>(
				request, 
				R4MEInfrastructureSettings.ApiHost, 
				HttpMethodType.Put, 
				out errorString);
	   ...
	   where T : class
						...
						case HttpMethodType.Delete:
							{
                                ...
                                if ((response.IsSuccessStatusCode && response.StatusCode== HttpStatusCode.OK) || response.StatusCode==HttpStatusCode.RedirectMethod)
                                {
		...
		//private async Task<Tuple<T, string>> GetResponseFromAPIAsync<T>(GenericParameters optimizationParameters,
		//							   string url,
		//							   HttpMethodType httpMethod,
		//							   HttpContent httpContent,
		//							   bool isString)
