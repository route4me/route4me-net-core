...
namespace Route4MeSDK.Examples
{
    public sealed partial class Route4MeExamples
    {
        ...
        /// </summary>
        public void CreateAndDeleteScheduleForRoute()
        {
            // Prepare routes
            var routes = CreateRoute();

            var route4Me = new SchedulesManagerV5(ActualApiKey);
            ...
            var routeId = routes.First().RouteID;

            // Create schedule for route
            var createRouteScheduleResponse = route4Me.CreateRouteSchedule(
                new RouteSchedule()
                { RouteId = routeId, ScheduleUid = scheduleResponse.Data.Single().ScheduleUid },
                out resultResponse);

            // Read schedule for route
            var response = route4Me.GetRouteSchedule(routeId, out resultResponse);

            // Delete schedule for route
            var deleteResponse = route4Me.DeleteRouteSchedule(routeId, out resultResponse);

            // Read no schedules for route
            response = route4Me.GetRouteSchedule(routeId, out resultResponse);
        ...
        private DataObjectRoute[] CreateRoute()
        {
            ...
            if (ownerId == null) return Array.Empty<DataObjectRoute>();
            ...
            var optimizationParameters = new QueryTypes.V5.OptimizationParameters()
            {
                Addresses = addresses,
