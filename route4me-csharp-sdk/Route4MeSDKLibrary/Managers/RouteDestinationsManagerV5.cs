...
namespace Route4MeSDKLibrary.Managers
{
    ...
    public class RouteDestinationsManagerV5 : Route4MeManagerBase
    {
        ...
        /// Includes column configuration for display in data-table views.
        /// Uses POST /route-destinations/list/combined.
        /// </summary>
        /// <param name="request">Filter, pagination and field-selection parameters.</param>
        /// <param name="resultResponse">Failure response if the request fails.</param>
        /// <returns>
        /// A <see cref="RouteDestinationsCombinedResource"/> with items, pagination and config,
        /// or <c>null</c> on failure.
        /// </returns>
            ...
            out ResultResponse resultResponse)
        {
            ...
            return GetJsonObjectFromAPI<RouteDestinationsCombinedResource>(
                request,
                R4MEInfrastructureSettingsV5.RouteDestinationsListCombined,
                HttpMethodType.Post,
                out resultResponse);
        }
        ...
        /// <param name="request">Filter, pagination and field-selection parameters.</param>
        /// <returns>
        /// A <see cref="Tuple{T1,T2}"/> containing the combined resource and failure response (if any).
        /// </returns>
        public async Task<Tuple<RouteDestinationsCombinedResource, ResultResponse>> GetDestinationsCombinedAsync(
            GetDestinationsRequest request)
        {
            if (request == null)
            {
                return new Tuple<RouteDestinationsCombinedResource, ResultResponse>(null, new ResultResponse
                {
                    Status = false,
                    Messages = new Dictionary<string, string[]>
                    {
            ...
            var result = await GetJsonObjectFromAPIAsync<RouteDestinationsCombinedResource>(
                request,
                R4MEInfrastructureSettingsV5.RouteDestinationsListCombined,
                HttpMethodType.Post).ConfigureAwait(false);
